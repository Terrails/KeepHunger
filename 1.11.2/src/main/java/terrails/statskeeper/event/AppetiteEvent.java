...
public class AppetiteEvent {
    ...
    {
        if ((ConfigHandler.noEating) && (event.getWorld().getBlockState(event.getPos()).getBlock() instanceof BlockPamCake) && (event.getEntityPlayer().isPotionActive(ModPotions.appetite))) {
            event.setCanceled(true);
        }
    }
